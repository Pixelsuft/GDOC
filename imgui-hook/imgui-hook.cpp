...
void _stub() {}
void _stub1(WPARAM param1, bool is_down) {}
void _stub2(HWND hwnd) {}
std::function<void()> g_drawFunc = _stub;
std::function<void(WPARAM param1, bool is_down)> g_toggleCallback = _stub1;
std::function<void(HWND hwnd)> g_getHwnd = _stub2;

void ImGuiHook::setRenderFunction(std::function<void()> func) {
    g_drawFunc = func;
}

void ImGuiHook::setToggleCallback(std::function<void(WPARAM param1, bool is_down)> func) {
    g_toggleCallback = func;
}

void ImGuiHook::setHwndCallback(std::function<void(HWND hwnd)> func) {
    g_getHwnd = func;
}
...
void __fastcall CCEGLView_swapBuffers_H(CCEGLView* self) {
    ...
    if (!g_inited) {
        ...
        ImGui::CreateContext();
        ImGui::GetIO();
        HWND hwnd = WindowFromDC(*reinterpret_cast<HDC*>(reinterpret_cast<uintptr_t>(window) + 0x244));
...
void __fastcall CCEGLView_pollEvents_H(CCEGLView* self) {
    ...
    while (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
        ...
        if (io.WantCaptureKeyboard) {
            ...
            g_toggleCallback(msg.wParam, true);
        }
        else if (msg.message == WM_KEYUP) {
            g_toggleCallback(msg.wParam, false);
        }
